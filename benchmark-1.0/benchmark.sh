...
if [ ! -z "$(ffmpeg -encoders 2>/dev/null | grep h264_omx)" ]; then
   zstd -k -o /tmp/Bosphorus_3840x2160.y4m "$DATA_DIR/Bosphorus_3840x2160.y4m.zst"
...
openssl speed -multi "$threadsCount" "${opensslCiphers[@]}" | tee "$RESULT_DIR/openssl_multi.txt"
